...
plot_data_BW_primiparous <- plot_m_BW_primiparous_time %>%
  mutate(x=round(x,0)) %>%
  left_join(x_date) %>%
...
## test with multiparous women only
```{r}
m_BW_multiparous <- bam(BW ~ s(GA_weeks) +  s(mat_age) + s(birth_Y_M_num) + s(month, bs="cc") + s(mean_ssep2) + s(mean_Alt_mean2)+ sex + Urban3 + Language + mother_nationality_cat2 + civil_status3 + HW + GR + COVID_first_wave + COVID_second_wave, data=bevn_eco_in7_multiparous) 
  summary(m_BW_multiparous)
  gam.check(m_BW_multiparous) 
plot_m_BW_multiparous_time <- GAM_plot_function_continuous(m_BW_multiparous, 3) #time
```

### Graph time X birthweight

```{r}
x_date <-   bevn_eco_in7_multiparous %>% #careful, mayhave to change the dataset if I later work with smaller time ranges!
  select(birth_Y_M, birth_Y_M_num) %>%
  distinct(birth_Y_M, .keep_all = TRUE) %>%
  rename(x=birth_Y_M_num)

plot_data_BW_multiparous <- plot_m_BW_multiparous_time %>%
  mutate(x=round(x,0)) %>%
  left_join(x_date) %>%
  mutate(date_x= ymd(paste0(birth_Y_M,"-01")))
...
plot_data_BW_multiparous_time <- ggplot(data=plot_data_BW_multiparous)+
  geom_line(aes(x=as.POSIXct(date_x),y=fit),lwd=lwdline, color=("#9557DC"))+
  geom_ribbon(aes(x=as.POSIXct(date_x),ymin=CIl,ymax=CIu),alpha=0.15,lwd=lwdline, fill='#69D8F3')+
  xlab("Timeline (years)")+
  ylab("Birthweight (g)")+
  ...
  theme(axis.text.y=element_text(color="black",size=size_axis),
        ...
        legend.title=element_text(size=size_legend_title),
        legend.position="bottom") +
  ggtitle("Birthweight through time, among multiparous women")
plot_data_BW_multiparous_time
```

### Table summarizing the birthweight outcome model

```{r}
  beta <- coef(m_BW_multiparous)
  Vb <- vcov(m_BW_multiparous, unconditional = TRUE)
  se <- sqrt(diag(Vb))
  n <- 584156
  d <- (abs(beta)/(sqrt(n)*se))
  my.ci <- data.frame(cbind(beta, lci = beta-1.96*se, uci = beta + 1.96*se, d))
  tab_BW_multiparous <- round(head(my.ci,15), 2)
```

```{r}
tab_BW_multiparous['variable'] <- c("Intercept", 
                              "Sex (ref: male)", "Urban (ref: rural)",
                              "Language region (ref: German)", " ", 
                              "Maternal nationality (ref: Swiss)", " ", " ",
                              " ", " ",
    ...
    tab_BW_multiparous['category'] <- c(" ", "Female", "Urban", "French", "Italian", 
                            "Africa", "Asia", "Europe", "Northern America",
                            "Southern/Central America", "Single", " ", " "," ", " ")
    tab_BW_multiparous<-tab_BW_multiparous[, c('variable', "category", "beta", "lci","uci", "d")]

BW_multiparous_gt <- tab_BW_multiparous %>%
  gt()%>%
  tab_header(title="Birthweight GAM, among multiparous women") %>%
  tab_spanner(label = "95% CI (g)", columns = c(lci, uci)) %>%
  tab_style(
    style = list(
          cell_fill(color = "#E0E0E0")
...
## PTB: test with multiparous women only
```{r}
m_PTB_multiparous <- bam(PTB~ s(BW) + s(mat_age) + s(birth_Y_M_num) + s(month, bs="cc") + s(mean_ssep2) + s(mean_Alt_mean2) + sex + Urban3 + Language + mother_nationality_cat2 + civil_status3 + HW + GR + COVID_first_wave + COVID_second_wave, family="binomial", data=bevn_eco_in7_multiparous) 
  summary(m_PTB_multiparous)

plot_m_PTB_multiparous_time <- GAM_plot_function_continuous(m_PTB_multiparous, 3) #time
```
...
plot_data_PTB_multip <- plot_m_PTB_multiparous_time %>%
  mutate(x=round(x,0)) %>%
  left_join(x_date) %>%
  mutate(date_x= ymd(paste0(birth_Y_M,"-01")))
lim1 <- as.POSIXct(ymd("2007-01-01"))
...
plot_PTB_multip <- ggplot(data=plot_data_PTB_multip)+
...
```{r}
  OR <- exp(coef(m_PTB_multiparous))
  beta <- coef(m_PTB_multiparous)
  Vb <- vcov(m_PTB_multiparous, unconditional = TRUE)
  se <- sqrt(diag(Vb))
  lci <- exp(beta-1.96*se)
  uci <- exp(beta+1.96*se)
  d <- abs(beta)/1.81
  my.ci <- data.frame(cbind(OR, lci, uci, d))
  tab_m_PTB_multiparous <- round(head(my.ci, 15), 2)
```

```{r}
tab_m_PTB_multiparous['variable'] <- c("Intercept", 
                              "Sex (ref: male)", "Urban (ref: rural)",
                              "Language region (ref: German)", " ", 
                              "Maternal nationality (ref: Swiss)", " ", " ",
                              " ", " ",
    ...
    tab_m_PTB_multiparous['category'] <- c(" ", "Female", "Urban", "French", "Italian", 
                            "Africa", "Asia", "Europe", "Northern America",
                            "Southern/Central America", "Single", " ", " "," ", " ")
    tab_m_PTB_multiparous<-tab_m_PTB_multiparous[, c('variable', "category", "OR", "lci","uci", "d")]

PTB_multiparous_gt <- tab_m_PTB_multiparous %>%
  gt()%>%
  tab_header(title="Preterm birth GAM, among multiparous women") %>%
  tab_spanner(label = "95% CI", columns = c(lci, uci)) %>%
  tab_style(
    style = list(
          cell_fill(color = "#E0E0E0")
