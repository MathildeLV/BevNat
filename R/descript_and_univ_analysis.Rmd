...
round(prop.table(table(bevn_eco_in3$stillbirth, useNA="always"))*100,3)
```
...
#BW categ. in LBW (<2500g) "normal" (arbitrary decision=2500-5000g) and high BW (>5000)
table(bevn_eco$BW_cat, useNA="always")
round(prop.table(table(bevn_eco$BW_cat, useNA="always"))*100, 3)
table(bevn_eco_in7$BW_cat, useNA="always")
round(prop.table(table(bevn_eco_in7$BW_cat, useNA="always"))*100, 3)
```
...
```{r}
table(bevn_eco_in7$com1, bevn_eco_in7$mother_residence_cat1, useNA = "always")
```
...
```{r}
 ...
 # BW category and language region
  ...
  round(prop.table(table(bevn_eco$BW_cat,bevn_eco$Language, useNA="always"), margin=2)*100,2)
  table(bevn_eco_in7$BW_cat, bevn_eco_in7$Language)
  round(prop.table(table(bevn_eco_in7$BW_cat,bevn_eco_in7$Language, useNA="always"), margin=2)*100,2)
```
...
plot(GA_freq$birthyear, GA_freq$GA_weeks)
round(prop.table(table(bevn_eco_in7$GA_weeks_cat, bevn_eco_in7$birthyear, useNA="always"), margin=2)*100,2)
...
```{r}
table(bevn_eco_in3$stillbirth, bevn_eco_in3$birthyear, useNA="always")
round(prop.table(table(bevn_eco_in3$stillbirth, bevn_eco_in3$birthyear), margin=2)*100,3)

#calculating median GA by year of birth, for stillborn, with tidyverse
  GA_SB_Y <- bevn_eco_in3 %>%
  group_by(birthyear) %>%
  filter(stillbirth==1) %>% 
   filter(!is.na(GA_weeks)) %>% 
    summarise(medGA=format(median(GA_weeks),digits=5))   
...
```{r}
 #calculating median BW for stillborn babies, by year
  BW_median_sb <- aggregate(BW ~ birthyear, data = subset(bevn_eco_in3, stillbirth==1), median) 
  BW_median_sb
  
 #calculating mode BW for stillborn babies, by year
  BW_y_mode <- bevn_eco_in3 %>%
   group_by(birthyear) %>%
     filter( !is.na(BW)) %>%
     filter(stillbirth==1) %>% 
 ...
 boxplot(BW~birthyear, data=bevn_eco_in3, subset=BW<8000)
```
...
```{r}
#month year
  tail(bevn_eco_in7$birthdate3)
  head(bevn_eco_in7$birthdate3)

  #day month year
  summary(bevn_eco_in7$birthdate2)
  tail(bevn_eco_in7$birthdate2)
  head(bevn_eco_in7$birthdate2)
```
...
plot(bevn_eco$BW, bevn_eco$BL) #BW and BL, all
...
m_BW_time2 <- lm(BW~birthdate2, data=bevn_eco_in7)
summary(m_BW_time2)

m_BW_time3_num <- lm(BW~birthdate3_num, data=bevn_eco_in7)
...
BW is rather impacted by parity.
...
m_SB_mat_age <- glm(stillbirth~mat_age, 
                    family= "binomial",data=bevn_eco_in3)
