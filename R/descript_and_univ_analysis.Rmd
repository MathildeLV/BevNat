...
# Characteristics of the population (whole table)

```{r}
tab1 <- bevn_eco_in7 %>%
  group_by(birthyear) %>%
summarise(mean_birthweight = round(mean(BW, na.rm = T), digits = 2),
            sd_birthweight = round(sd(BW, na.rm = T), digits = 2),
            count_females = sum(sex == "2"),
            n = n(),
            percent_females = count_females/n,
        count_males = sum(sex == "1"),
            percent_males = count_males/n)%>%
  
  ungroup()
  tab1
  
formatted <- tab1 %>%
  mutate(birthweight = stringr::str_glue("{mean_birthweight} ± {scales::percent(sd_birthweight)}"),
         females = stringr::str_glue("{scales::percent(percent_females, accuracy = 1)}")) %>%
  tidyr::complete(birthyear) %>%
  select(birthyear, birthweight, females)

knitr::kable(formatted)
  
 #  formatted <- tab1 %>%
 # mutate(birthweight = stringr::str_glue("{mean_birthweight} ± {scales::percent(sd_birthweight)}"),
#         sex = stringr::str_glue("{scales::percent(percent_females, accuracy = 1)} ({count_females} / #{n})")) %>%
 # tidyr::complete(birthyear) %>%
  #select(birthyear, birthweight, sex)
#knitr::kable(formatted)
...
bevn_eco_in7[,list(
  meanBirthweight = mean(BW),
  BirthweightSd = sd(BW),
  females = 100*sum(sex == "2")/.N,
  males = 100*sum(sex=="1")/.N),
...
summary(m_BW_time2)
m_BW_time2_num <- lm(BW~birthdate2_num, data=bevn_eco_in7)
summary(m_BW_time2_num)

m_BW_time3<- lm(BW~birthdate3_num, data=bevn_eco_in7)
summary(m_BW_time3)
...
m_SB_birthday <- glm(stillbirth~birthdate2_num, 
                    family= "binomial",data=bevn_eco_in6)
summary(m_SB_birthday) 

m_SB_birthmonth <- glm(stillbirth~birthdate3_num, 
                    family= "binomial",data=bevn_eco_in6)
