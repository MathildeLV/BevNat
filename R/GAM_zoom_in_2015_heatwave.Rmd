---
title: "Heatwave 2015 : zooming in 2014-16"
...
m_BW_bam_HW_2015 <- bam(BW ~ s(GA_weeks) +  s(mat_age) + s(birth_Y_M_num) + s(mean_ssep2) + s(mean_Alt_mean2)+ parity_cat + sex + Urban3 + Language + mother_nationality_cat2 + civil_status3, data=bevn_eco_in7_2014_16) 
  ...
  plot(m_BW_bam_HW_2015, select=4, ylim=c(3325, 3345),shade = TRUE, shade.col = "lightblue",  shift = coef(m_BW_bam_HW_2015)[1])
  plot(m_BW_bam_HW_2015, select=5, ylim=c(3200, 3400),shade = TRUE, shade.col = "lightblue",  shift = coef(m_BW_bam_HW_2015)[1])
  plot(m_BW_bam_HW_2015, select=6, ylim=c(3100, 3500),shade = TRUE, shade.col = "lightblue",  shift = coef(m_BW_bam_HW_2015)[1])
