...
## SB % stratified by SSEP tertile, adjusted for time only
```{r include=FALSE}
m_SB_bam_L_SSEP1_sp <- bam(stillbirth ~ s(birth_Y_M_num), family="binomial", data=bevn_eco_in6_L_SSEP1)  
m_SB_bam_M_SSEP1_sp <- bam(stillbirth ~ s(birth_Y_M_num), family="binomial", data=bevn_eco_in6_M_SSEP1)  
m_SB_bam_H_SSEP1_sp <- bam(stillbirth ~ s(birth_Y_M_num), family="binomial", data=bevn_eco_in6_H_SSEP1)  

#function to extract the plot information
plot_m_SB_bam_L_SSEP1_sp <- GAM_plot_function_binary(m_SB_bam_L_SSEP1_sp, 1) %>%
  mutate(SEP="low")
plot_m_SB_bam_M_SSEP1_sp <- GAM_plot_function_binary(m_SB_bam_M_SSEP1_sp, 1) %>%
  mutate(SEP="medium")
plot_m_SB_bam_H_SSEP1_sp <- GAM_plot_function_binary(m_SB_bam_H_SSEP1_sp, 1) %>%
  mutate(SEP="high")
```
...
plot_SB_SEP_sp <- plot_m_SB_bam_L_SSEP1_sp  %>%
  rbind(plot_m_SB_bam_M_SSEP1_sp) %>%
  rbind(plot_m_SB_bam_H_SSEP1_sp) %>%
  mutate(x=round(x,0)) %>%
  left_join(x_date) %>%
...
ggplot_SB_SEP_sp <- ggplot(data=plot_SB_SEP_sp)+
  ...
  ggtitle("Stillbirth rate through time, stratified by SSEP, unadj. model")
ggplot_SB_SEP_sp
```
...
ggplot_SB_SEP <- ggplot(data=plot_SB_SEP)+
  ...
  theme(axis.text.y=element_text(color="black",size=size_axis),
    ...
    panel.grid.major=element_line(colour = "black", linetype = "dotted"),
    panel.grid.minor=element_line(colour = "black", linetype = "dotted"))+ 
  ggtitle("Stillbirth rate through time, stratified by SSEP")
ggplot_SB_SEP
```
...
## SB % stratified by maternal nationality, adjusted for time only
```{r include=FALSE}
m_SB_bam_swiss_sp <- bam(stillbirth ~ s(birth_Y_M_num), family="binomial", data=bevn_eco_in6_Swiss)  
m_SB_bam_Afr_sp <- bam(stillbirth ~ s(birth_Y_M_num), family="binomial", data=bevn_eco_in6_Afr)
m_SB_bam_Asi_sp <- bam(stillbirth ~ s(birth_Y_M_num), family="binomial", data=bevn_eco_in6_Asi)
m_SB_bam_Eur_sp <- bam(stillbirth ~ s(birth_Y_M_num), family="binomial", data=bevn_eco_in6_Eur)  
m_SB_bam_Nort_Am_sp <- bam(stillbirth ~ s(birth_Y_M_num), family="binomial", data=bevn_eco_in6_Nort_Am)  
m_SB_bam_S_C_Am_sp <- bam(stillbirth ~s(birth_Y_M_num), family="binomial", data=bevn_eco_in6_S_C_Am)  

plot_m_SB_bam_swiss_sp <- GAM_plot_function_binary(m_SB_bam_swiss_sp, 1) %>%
  mutate(nationality="Swiss")
plot_m_SB_bam_Afr_sp <- GAM_plot_function_binary(m_SB_bam_Afr_sp, 1) %>%
  mutate(nationality="African")
plot_m_SB_bam_Asi_sp <- GAM_plot_function_binary(m_SB_bam_Asi_sp, 1) %>%
  mutate(nationality="Asian")
plot_m_SB_bam_Eur_sp <- GAM_plot_function_binary(m_SB_bam_Eur_sp, 1) %>%
  mutate(nationality="European")
plot_m_SB_bam_Nort_Am_sp <- GAM_plot_function_binary(m_SB_bam_Nort_Am_sp, 1) %>%
  mutate(nationality="Northern American")
plot_m_SB_bam_S_C_Am_sp <- GAM_plot_function_binary(m_SB_bam_S_C_Am_sp, 1) %>%
  mutate(nationality="Southern/Central American")
```

```{r}
plot_SB_nationality_sp <- plot_m_SB_bam_swiss_sp %>%
  rbind(plot_m_SB_bam_Afr_sp) %>%
  rbind(plot_m_SB_bam_Asi_sp) %>%
  rbind(plot_m_SB_bam_Eur_sp) %>%
  rbind(plot_m_SB_bam_Nort_Am_sp) %>%
  rbind(plot_m_SB_bam_S_C_Am_sp) %>%
  mutate(x=round(x,0)) %>%
  left_join(x_date) %>%
  mutate(date_x= ymd(paste0(birth_Y_M,"-01")))

ggplot_SB_nationality_sp <- ggplot(data=plot_SB_nationality_sp)+
  ...
  theme(axis.text.y=element_text(color="black",size=size_axis),
    ...
    panel.grid.major=element_line(colour = "black", linetype = "dotted"),
    panel.grid.minor=element_line(colour = "black", linetype = "dotted"))
ggplot_SB_nationality_sp+ 
  ggtitle("Stillbirth rate through time, stratified by maternal nationality, unadj. model")
```
...
ggplot_SB_nationality <- ggplot(data=plot_SB_nationality)+
  ...
  theme(axis.text.y=element_text(color="black",size=size_axis),
    ...
    panel.grid.major=element_line(colour = "black", linetype = "dotted"),
    panel.grid.minor=element_line(colour = "black", linetype = "dotted"))+ 
  ggtitle("Stillbirth rate through time, stratified by maternal nationality")
...
## SB % stratified by langage region, adjusted for time only
```{r include=FALSE}
m_SB_bam_German_sp<- bam(stillbirth ~ s(birth_Y_M_num), family="binomial", data=bevn_eco_in6_German) 
m_SB_bam_French_sp<- bam(stillbirth ~ s(birth_Y_M_num), family="binomial", data=bevn_eco_in6_French)  
m_SB_bam_Italian_sp<- bam(stillbirth ~ s(birth_Y_M_num), family="binomial", data=bevn_eco_in6_Italian)  
 
plot_m_SB_bam_German_sp <- GAM_plot_function_binary(m_SB_bam_German_sp, 1) %>%
  mutate(language_region="German")
plot_m_SB_bam_French_sp <- GAM_plot_function_binary(m_SB_bam_French_sp, 1) %>%
  mutate(language_region="French")
plot_m_SB_bam_Italian_sp <- GAM_plot_function_binary(m_SB_bam_Italian_sp, 1) %>%
  mutate(language_region="Italian")
```

```{r}
plot_SB_language_sp <- plot_m_SB_bam_German_sp %>%
  rbind(plot_m_SB_bam_French_sp) %>%
  rbind(plot_m_SB_bam_Italian_sp) %>%
  mutate(x=round(x,0)) %>%
  left_join(x_date) %>%
  mutate(date_x= ymd(paste0(birth_Y_M,"-01")))

ggplot_SB_Language_region_sp <- ggplot(data=plot_SB_language_sp)+
  ...
  theme(axis.text.y=element_text(color="black",size=size_axis),
    ...
    panel.grid.major=element_line(colour = "black", linetype = "dotted"),
    panel.grid.minor=element_line(colour = "black", linetype = "dotted"))+ 
  ggtitle("Stillbirth rate through time, stratified by language region, unadj. model")
ggplot_SB_Language_region_sp
```
...
ggplot_SB_Language_region <- ggplot(data=plot_SB_language)+
  ...
  theme(axis.text.y=element_text(color="black",size=size_axis),
    ...
    panel.grid.major=element_line(colour = "black", linetype = "dotted"),
    panel.grid.minor=element_line(colour = "black", linetype = "dotted"))+ 
  ggtitle("Stillbirth rate through time, stratified by language region")
