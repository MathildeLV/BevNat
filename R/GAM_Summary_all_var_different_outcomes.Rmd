...
```{r}
 pvalues_smooth_BW_model = summary(m_BW_bam5_2)
  ...
  BW_full_smooth_pvalues_gt <- pvalues_smooth_BW_model_df %>%
    ...
    gt()%>% 
    tab_header(title="Birthweight GAM, smooth variables") 
...
pvalues_smooth_SB_model = summary(m_SB_bam2)
  ...
  SB_full_smooth_pvalues_gt <- pvalues_smooth_SB_model_df %>%
    select(-one_of('pvalue')) %>%
    gt()%>% 
    tab_header(title="Stillbirth GAM, smooth variables") 
...
pvalues_smooth_PTB_model = summary(m_PTB_bam)
  ...
  PTB_full_smooth_pvalues_gt <- pvalues_smooth_PTB_model_df %>%
    select(-one_of('pvalue')) %>%
    gt()%>% 
    tab_header(title="Preterm birth GAM, smooth variables") 
...
```{r}
m_GA_bam <- bam(GA_weeks ~   s(mat_age) + s(birth_Y_M_num) + s(month, bs="cc") + s(mean_ssep2) + s(mean_Alt_mean2)+ parity_cat + sex + Urban3 + Language + mother_nationality_cat2 + civil_status3 + HW + GR + COVID_first_wave + COVID_second_wave, data=bevn_eco_in7) 
summary(m_GA_bam)
...
m_GA_bam <- bam(GA_weeks ~ s(GA_weeks) +  s(mat_age) + s(birth_Y_M_num) + s(month, bs="cc") + s(mean_ssep2) + s(mean_Alt_mean2)+ parity_cat + sex + Urban3 + Language + mother_nationality_cat2 + civil_status3 + HW + GR + COVID_first_wave + COVID_second_wave, data=bevn_eco_in7) 
