...
## Birthweight
```{r}
m_BW1 <- lm(BW ~ GA_weeks + birthdate2 + parity_cat + sex, data=bevn_eco_in7)
summary(m_BW1)

  #with mat age 
m_BW2 <- lm(BW ~ GA_weeks + birthdate2 + mat_age + parity_cat + sex, data=bevn_eco_in7)
  summary(m_BW2)
 
  #with mat age category
m_BW2_matage_cat <- lm(BW ~ GA_weeks + birthdate2 + mat_age_cat + parity_cat + sex, data=bevn_eco_in7)
  summary(m_BW2_matage_cat)
  
  #with mother nationality (categorical 1)
   m_BW2_mat_nat_cat1 <- lm(BW ~ mother_nationality_cat1 + GA_weeks + birthdate2 + mat_age_cat + parity_cat + sex, data=bevn_eco_in7)
  summary(m_BW2_mat_nat_cat1)
  
  #with mother nationality (categorical 2)
   m_BW2_mat_nat_cat2 <- lm(BW ~ mother_nationality_cat2 + GA_weeks + birthdate2 +  mat_age_cat + parity_cat + sex, data=bevn_eco_in7)
  summary(m_BW2_mat_nat_cat2)
  
  #with urban category (categorical 3)
   m_BW2_urb_cat_3 <- lm(BW ~ Urban3 + mother_nationality_cat2 + GA_weeks + birthdate2 +  mat_age_cat + parity_cat + sex, data=bevn_eco_in7)
  summary(m_BW2_urb_cat_3)  
  
  #with month
m_BW2_month <- lm(BW ~ month + Urban3 + mother_nationality_cat2 + GA_weeks + birthdate2 +  mat_age_cat + parity_cat + sex, data=bevn_eco_in7)
  summary(m_BW2_month)
...
## Stillbirth
```{r}
m_SB_multiv_1 <- glm(stillbirth~mat_age_cat + GA_weeks + birthdate3_num + mother_nationality_cat1, 
                    family= "binomial",data=bevn_eco_in6)
summary(m_SB_multiv_1) 
exp(coef(summary(m_SB_multiv_1)))[,"Estimate"]

m_SB_multiv_2 <- glm(stillbirth~mat_age_cat + GA_weeks + birthdate3_num + mother_nationality_cat2, 
                    family= "binomial",data=bevn_eco_in6)
summary(m_SB_multiv_2) 
exp(coef(summary(m_SB_multiv_2)))[,"Estimate"]

m_SB_multiv_3 <- glm(stillbirth~mat_age_cat + GA_weeks + birthdate3_num + mother_nationality_cat2 + Urban3,  family= "binomial",data=bevn_eco_in6)
...
```{r}
m_BW_bam2 <- bam(BW ~ s(GA_weeks) +  s(mat_age) + s(birthdate3_num) + parity_cat + sex + Urban3 + mother_nationality_cat1, data=bevn_eco_in7)
  summary(m_BW_bam2)
plot(m_BW_bam2, all.terms = TRUE,pages=1, shade = TRUE, shade.col = "lightblue")


m_BW_bam3 <- bam(BW ~ s(GA_weeks) +  s(mat_age) + s(birthdate3_num) + month + parity_cat + sex + Urban3 + mother_nationality_cat1, data=bevn_eco_in7)
  summary(m_BW_bam3)
  
m_BW_bam4 <- bam(BW ~ s(GA_weeks) +  s(mat_age) + s(birthdate3_num) + s(month) + s(mean_ssep2) + parity_cat + sex + Urban3 + mother_nationality_cat1 + Language, data=bevn_eco_in7)
  summary(m_BW_bam4)
...
m_BW_bam4_1 <- bam(BW ~ s(GA_weeks) +  s(mat_age, k=13) + s(birthdate3_num) + s(month) + parity_cat + sex + Urban3 + mother_nationality_cat1, data=bevn_eco_in7)
summary(m_BW_bam4_1)
  gam.check(m_BW_bam4_1) #pvalue not significant, k-index for month and birthdate is < 1.

#try with mat age k=12 and month k=10
  m_BW_bam4_2 <- bam(BW ~ s(GA_weeks) +  s(mat_age, k=13) + s(birthdate3_num) + s(month, k=11) + parity_cat + sex + Urban3 + mother_nationality_cat1, data=bevn_eco_in7)
summary(m_BW_bam4_2)
...
```
## GAM with Stillbirth as outcome variable
