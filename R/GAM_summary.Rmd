...
m_BW_bam5_2 <- bam(BW ~ s(GA_weeks) +  s(mat_age) + s(birth_Y_M_num) + s(month, bs="cc") + s(mean_ssep2) + s(mean_Alt_mean2)+ parity_cat + sex + Urban3 + Language + mother_nationality_cat2 + civil_status3 + HW + GR + COVID_first_wave + COVID_second_wave, data=bevn_eco_in7) 
  ...
  plot(m_BW_bam5_2, select=5, ylim=c(3200, 3400),shade = TRUE, shade.col = "lightblue",  shift = coef(m_BW_bam5_2)[1])
    plot(m_BW_bam5_2, select=6, ylim=c(3100, 3500),shade = TRUE, shade.col = "lightblue",  shift = coef(m_BW_bam5_2)[1])
```
...
Because birth_Y\_M_num and month variables correlated too much, month variable was not kept in the model. This was done in every "sub" model including only the years 2008-2010 or 2017-2020.
...
#birth_Y_M, month and mean ssep as linear variables:
  ...
  plot(m_PTB_bam_covid_2, trans = plogis, select=2, ylim=c(0, 0.04),shade = TRUE, shade.col = "lightblue",  shift = coef(m_PTB_bam_covid_2)[1]) #mat age
...
# Macrosomia defined as BW>95th percentile
## 2007-2020
```{r}
#normal regression
m_macro_95th <- rq(BW ~ GA_weeks+ mat_age +birth_Y_M_num +month + mean_ssep2 + parity_cat + Urban3 + Language + mean_Alt_mean2 + sex + Urban3 + Language + mother_nationality_cat2 + civil_status3 + HW, data = bevn_eco_in7, tau = 0.95)

summary(m_macro_95th)
```
