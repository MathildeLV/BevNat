...
bevn_eco <- bevn_eco %>%
  ...
  dplyr::rename(pat_age=Vater..Alter.in.erfüllten.Jahren,
                sex=Kind..Geschlecht,
                GA_days=Kind..Gestationsalter.in.Tagen,
                country_of_birth=Geburtsstaat,
                nb_of_babies=Art.der.Geburt,
                BL=Kind..Grösse.in.Zentimeter,
                BW=Kind..Gewicht.in.Gramm,
                mother_nationality=Mutter..Staatsangehörigkeit,
                civil_status=Mutter.Zivilstand,
                father_nationality=Vater..Staatsangehörigkeit,
                parity=Kind..biologischer.Rang,
                resident_status=Mutter..ständig.oder.nicht.ständiger.Wohnsitz)
...
## Mutter: Wohngemeinde / Wohnstaat = Place of residence Mother
 ...
 #Urban3 as categ
  bevn_eco <- bevn_eco %>%
   mutate(Urban3=as.factor(Urban3))
...
# Birthweight categorizations
  bevn_eco <- bevn_eco %>%
    mutate(macro = ifelse(BW>4000,1,0), #Macro vs the rest
           macro=as.factor(macro),
           LBW = ifelse(BW<2500,1,0), # LBW vs the rest
           LBW=as.factor(LBW),
           BW_cat2=ifelse(BW<500,1,0), # for stillbirth definition
           BW_cat2=as.factor(BW_cat2),
           VLBW = ifelse(BW<1000,1,0), # VLBW vs the rest
           VLBW=as.factor(VLBW),
           LBW_norm = case_when (BW<2500 & BW>0 ~"1", #LBW vs normal
                                 BW<4000 & BW>2500 | BW ==2500 ~"0"),
           LBW_norm=as.factor(LBW_norm),
           macro_norm = case_when (BW>4000 | BW==4000~"1", #Macro vs normal
                            BW<4000 & BW>2500 | BW ==2500 ~"0"),
           macro_norm=as.factor(macro_norm),
           BW_ordered = case_when (BW<2500 & BW>0 ~"Low birthweight", # LBW vs normal vs Macro
                            BW<4000 & BW>2500 ~"Normal birthweight",
                            BW==2500 ~"Normal birthweight",
                            BW>4000 | BW==4000~"Macrosomia"),
            BW_ordered=as.factor(BW_ordered))
  table(bevn_eco$macro, useNA="always")
  table(bevn_eco$LBW, useNA="always")
  table(bevn_eco$VLBW, useNA="always")
  table(bevn_eco$LBW_norm, useNA="always")
  table(bevn_eco$macro_norm, useNA="always")
  table(bevn_eco$BW_ordered, useNA="always")

  ## BL = BL (cm)
  bevn_eco$BL_cat <- cut(bevn_eco$BL, breaks=c(0, 20, 65, 100), include.lowest=TRUE, right=FALSE)
...
# Dividing altitude and SSEP to interpret more easily  
  bevn_eco <- bevn_eco %>%
...
# NEW VARIABLES
  ...
  #GA categories
  bevn_eco <- bevn_eco %>%
    mutate(GA_weeks_cat = cut (GA_weeks, breaks=c(16,24,30,34,39,44,46), include.lowest=TRUE,  right = FALSE),  
  GA_weeks_cat2=cut(GA_weeks, breaks=c(10,22,46), include.lowest=TRUE,  right = FALSE ),
  GA_weeks_cat3=cut(GA_weeks, breaks=c(10,32,34,37,50), include.lowest=TRUE,  right = FALSE),
  PTB = case_when(GA_weeks <37 ~"1", GA_weeks>37 | GA_weeks==37 ~"0"),
        PTB = as.factor(PTB),
  VPTB = case_when(GA_weeks <32 ~"1", GA_weeks>32 | GA_weeks==32 ~"0"),
  VPTB = as.factor(VPTB),
  EPTB = case_when(GA_weeks==32 | (GA_weeks <34 & GA_weeks>32) ~"1", GA_weeks>34 | GA_weeks==34 ~"0"),
  EPTB=as.factor(EPTB))
  table(bevn_eco$GA_weeks_cat)
  ...
  table(bevn_eco$GA_weeks_cat3)
   ...
   # Maternal /paternal age categories
   table(bevn_eco$mat_age)
   bevn_eco <- bevn_eco %>%
     dplyr:: mutate(mat_age_cat = cut (mat_age, breaks=c(10,20,25,30, 35, 40, 70)),
                  mat_age_cat2 = case_when(mat_age <30 ~"0", mat_age>30 | mat_age==30 ~"1"),
                  mat_age_cat3 = case_when(mat_age <32 ~"0", mat_age>32 | mat_age==32 ~"1"),
                  pat_age_cat = cut (pat_age, breaks=c(10,20,30, 40, 50, 70, 100)))  
