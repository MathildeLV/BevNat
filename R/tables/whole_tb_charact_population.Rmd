...
summarise(n = n(),
    ...
    mean_birthweight = round(mean(BW, na.rm = T), digits = 0),   #birthweight
    sd_birthweight = round(sd(BW, na.rm = T), digits = 0),
...
formattedcomb0 <- tabcomb0 %>%
  mutate(n=n,
        ...
        "Birthweight (g)"= stringr::str_glue("{mean_birthweight} ± {sd_birthweight}"),    #birthweight
         "Gestational Age (weeks)"= stringr::str_glue("{mean_GA_weeks} ± {sd_GA_weeks}"),      #GA_weeks
  ...
  tidyr::complete(birthyear) %>%
  select(birthyear, "1", "2", "3", ">3", "missing", "Rural", "Urban", "missing Urban", "German or Romansh", "French", "Italian", "missing lang", "Switzerland", "Africa", "Asia", "Europe", "Northern America", "Southern and Central America", "Missing or Oceania", "Married", "Single", "Maternal age (years)", "Mean SSEP", "Mean altitude (MASL)", "Birthweight (g)", "Gestational Age (weeks)", female, male, Stillbirth, Preterm_birth, Low_birth_weight,
         n)
formattedcomb0
```
...
formattedcomb7 <- tabcomb7 %>%
  ...
  select(birthyear, "1", "2", "3", ">3", "missing",
         ...
         "German or Romansh", "French", "Italian", "missing lang",
         "Switzerland", "Africa", "Asia", "Europe", "Northern America", "Southern and Central America", "Missing or Oceania",
         "Married", "Single", "Maternal age (years)", "Mean SSEP", "Mean altitude (MASL)", "Birthweight (g)", "Gestational Age (weeks)", female, male, Preterm_birth, Low_birth_weight,
         n)
formattedcomb7
```
...
formattedcomb6 <- tabcomb6 %>%
  ...
  select(birthyear, "1", "2", "3", ">3", "missing",
         ...
         "German or Romansh", "French", "Italian", "missing lang",
         "Switzerland", "Africa", "Asia", "Europe", "Northern America", "Southern and Central America", "Missing or Oceania",
         "Married", "Single", "Maternal age (years)", "Mean SSEP", "Mean altitude (MASL)", 
         "Birthweight (g)", "Gestational Age (weeks)", female, male, Stillbirth, Preterm_birth, Low_birth_weight,
         n)
formattedcomb6

mat_neonat_in6_gt <- formattedcomb6%>%
  gt()%>%
  tab_header(title="maternal characteristics") %>%
  tab_spanner (label = "Parity", columns = c("1", "2", "3", ">3", "missing")) %>%
  tab_spanner(label = "Nationality 2", columns =c("Switzerland", "Africa", "Asia", "Europe", "Northern America", "Southern and Central America", "Missing or Oceania") ) %>%
...
tab_br <- bevn_eco_in_br %>%
  group_by(birthyear) %>%
summarise(n = n(),
          birth_rate= round(((1000*n)/mean(permanent_resident_population)), digits=2),
          )%>%
  ungroup()
  tab_br
...
# Birth rate whole pop° and by resident status

```{r}
# test <-  bevn_eco %>%
#   group_by(birthyear) %>%
#   mutate(birthrate_whole_pop=((1000*n())/(population)),
#        birthrate_perm_residents=((1000*n())/(permanent_resident_population)),
#        birthrate_non_perm_residents=((1000*n())/(non_permanent_population))) %>%
#   ungroup()
...
```{r}
bevn_eco_in7_GA <- bevn_eco_in7 %>%
  filter(GA_weeks >= 37) %>%
   mutate(GA_weeks_37_41=cut(GA_weeks, breaks=c(37,38,39,40,41,46), include.lowest=TRUE,  right = FALSE ))
table(bevn_eco_in7_GA$GA_weeks_37_41, useNA = "always")

tabd <- bevn_eco_in7_GA %>%
  group_by(birthyear) %>%
summarise(n = n(),
    count_GA_37= sum(GA_weeks_37_41=="[37,38)"),  perc_GA_37= 100*count_GA_37/n,     #parity
    count_GA_38= sum(GA_weeks_37_41=="[38,39)"), perc_GA_38= 100*count_GA_38/n,
    count_GA_39= sum(GA_weeks_37_41=="[39,40)"), perc_GA_39= 100*count_GA_39/n,
    count_GA_40= sum(GA_weeks_37_41=="[40,41)"),  perc_GA_40= 100*count_GA_40/n,
    count_GA_41 = sum(GA_weeks_37_41=="[41,46]"),  perc_GA_41= 100*count_GA_41/n,
   ) %>%     
  ungroup()
  tabd
...
formattedd <- tabd %>%
  mutate(n=round(n, digits=0),
        "37-38" = round({perc_GA_37}, digits=1), 
        "38-39" = round({perc_GA_38}, digits=1),
        "39-40" = round({perc_GA_39}, digits=1),
        "40-41" = round({perc_GA_40}, digits=1),
        ">=41" = round({perc_GA_41}, digits=1),
         ) %>%
  tidyr::complete(birthyear) %>%
  select(birthyear, "37-38", "38-39", "39-40", "40-41", ">=41")
formattedd
```


# Maternal nationality and language region
```{r}
tab_nat_lang <- bevn_eco_in6 %>%
  group_by(Language) %>%
summarise(n = n(),       
    #moth nat cat 2
    count_mat_nat_cat_2_S= sum(mother_nationality_cat2=="Switzerland", na.rm = T), perc_mat_nat_cat_2_S=100*count_mat_nat_cat_2_S/n,
...
formatted_nat_lang <- tab_nat_lang %>%
  mutate(n=n,
  ...
  tidyr::complete(Language) %>%
  select(Language, "Switzerland", "Africa", "Asia", "Europe", "Northern America", "Southern and Central America", "Missing or Oceania", n)
formatted_nat_lang


mat_nat_lang_gt <- formatted_nat_lang%>%
  gt()%>%
  tab_header(title="maternal characteristics") %>%
  tab_spanner(label = "Nationality", columns =c("Switzerland", "Africa", "Asia", "Europe", "Northern America", "Southern and Central America", "Missing or Oceania"))
mat_nat_lang_gt
```
